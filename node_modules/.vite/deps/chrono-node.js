...
var DECasualTimeParser = class _DECasualTimeParser extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    const targetDate = (0, import_dayjs12.default)(context.refDate);
...
var DECasualDateParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    let targetDate = (0, import_dayjs14.default)(context.refDate);
...
var FRCasualDateParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    let targetDate = (0, import_dayjs16.default)(context.refDate);
    const lowerText = match[0].toLowerCase();
    const component = context.createParsingComponents();
    switch (lowerText) {
      case "maintenant":
...
var JPStandardParser = class {
  ...
  extract(context, match) {
    ...
    if (match[SPECIAL_YEAR_GROUP] && match[SPECIAL_YEAR_GROUP].match("同|今|本")) {
      const moment = (0, import_dayjs18.default)(context.refDate);
...
var JPCasualDateParser = class {
  ...
  extract(context, match) {
    ...
    const date = (0, import_dayjs19.default)(context.refDate);
    const components = context.createParsingComponents();
    switch (text) {
      case "昨日":
        return yesterday(context.reference);
...
var PATTERN22 = new RegExp(`(?:(?:\\,|\\(|\\（)\\s*)?(?:(este|esta|passado|pr[oó]ximo)\\s*)?(${matchAnyPattern(WEEKDAY_DICTIONARY4)})(?:\\s*(?:\\,|\\)|\\）))?(?:\\s*(este|esta|passado|pr[óo]ximo)\\s*semana)?(?=\\W|\\d|$)`, "i");
var PREFIX_GROUP4 = 1;
...
var PATTERN23 = new RegExp(`([0-9]{1,2})(?:º|ª|°)?(?:\\s*(?:desde|de|\\-|\\–|ao?|\\s)\\s*([0-9]{1,2})(?:º|ª|°)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(${matchAnyPattern(MONTH_DICTIONARY4)})(?:\\s*(?:de|,)?\\s*(${YEAR_PATTERN4}))?(?=\\W|$)`, "i");
var DATE_GROUP6 = 1;
...
var PTMonthNameLittleEndianParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP9]) {
      const yearNumber = parseYear4(match[YEAR_GROUP9]);
      result.start.assign("year", yearNumber);
    } else {
      const year3 = findYearClosestToRef(context.refDate, day, month);
      result.start.imply("year", year3);
...
var NLMonthNameMiddleEndianParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP10]) {
      const year3 = parseYear5(match[YEAR_GROUP10]);
      components.assign("year", year3);
    } else {
      const year3 = findYearClosestToRef(context.refDate, day, month);
      components.imply("year", year3);
...
var NLSlashMonthFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    const year3 = parseInt(match[YEAR_GROUP12]);
    const month = parseInt(match[MONTH_GROUP3]);
    return context.createParsingComponents().imply("day", 1).assign("month", month).assign("year", year3);
  }
};
...
var NLCasualDateTimeParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    const component = context.createParsingComponents();
    const targetDate = (0, import_dayjs25.default)(context.refDate);
    switch (dateText) {
      case "gisteren":
...
var NLRelativeDateFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    const components = context.createParsingComponents();
    let date = (0, import_dayjs26.default)(context.reference.instant);
    if (unitWord.match(/week/i)) {
      date = date.add(-date.get("d"), "d");
      components.imply("day", date.date());
      components.imply("month", date.month() + 1);
...
var ZHHansDateParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    const startMoment = (0, import_dayjs27.default)(context.refDate);
    const result = context.createParsingResult(match.index, match[0]);
    let month = parseInt(match[MONTH_GROUP4]);
    if (isNaN(month))
      month = zhStringToNumber(match[MONTH_GROUP4]);
    result.start.assign("month", month);
    if (match[DAY_GROUP3]) {
      let day = parseInt(match[DAY_GROUP3]);
      if (isNaN(day))
        day = zhStringToNumber(match[DAY_GROUP3]);
      result.start.assign("day", day);
    } else {
      result.start.imply("day", startMoment.date());
    }
    if (match[YEAR_GROUP13]) {
      let year3 = parseInt(match[YEAR_GROUP13]);
      if (isNaN(year3))
        year3 = zhStringToYear(match[YEAR_GROUP13]);
      result.start.assign("year", year3);
    } else {
...
var PATTERN33 = new RegExp("(\\d+|[" + Object.keys(NUMBER).join("") + "]+|半|几)(?:\\s*)(?:个)?(秒(?:钟)?|分钟|小时|钟|日|天|星期|礼拜|月|年)(?:(?:之|过)?后|(?:之)?内)", "i");
var NUMBER_GROUP = 1;
var UNIT_GROUP = 2;
var ZHHansDeadlineFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    let date = (0, import_dayjs28.default)(context.refDate);
...
var PATTERN34 = new RegExp("(?<prefix>上|下|这)(?:个)?(?:星期|礼拜|周)(?<weekday>" + Object.keys(WEEKDAY_OFFSET).join("|") + ")");
var ZHHansRelationWeekdayParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    let startMoment = (0, import_dayjs29.default)(context.refDate);
...
var FIRST_REG_PATTERN3 = new RegExp("(?:从|自)?(?:(今|明|前|大前|后|大后|昨)(早|朝|晚)|(上(?:午)|早(?:上)|下(?:午)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|(今|明|前|大前|后|大后|昨)(?:日|天)(?:[\\s,，]*)(?:(上(?:午)|早(?:上)|下(?:午)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?)?(?:[\\s,，]*)(?:(\\d+|[" + Object.keys(NUMBER).join("") + "]+)(?:\\s*)(?:点|时|:|：)(?:\\s*)(\\d+|半|正|整|[" + Object.keys(NUMBER).join("") + "]+)?(?:\\s*)(?:分|:|：)?(?:\\s*)(\\d+|[" + Object.keys(NUMBER).join("") + "]+)?(?:\\s*)(?:秒)?)(?:\\s*(A.M.|P.M.|AM?|PM?))?", "i");
var SECOND_REG_PATTERN3 = new RegExp("(?:^\\s*(?:到|至|\\-|\\–|\\~|\\〜)\\s*)(?:(今|明|前|大前|后|大后|昨)(早|朝|晚)|(上(?:午)|早(?:上)|下(?:午)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|(今|明|前|大前|后|大后|昨)(?:日|天)(?:[\\s,，]*)(?:(上(?:午)|早(?:上)|下(?:午)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?)?(?:[\\s,，]*)(?:(\\d+|[" + Object.keys(NUMBER).join("") + "]+)(?:\\s*)(?:点|时|:|：)(?:\\s*)(\\d+|半|正|整|[" + Object.keys(NUMBER).join("") + "]+)?(?:\\s*)(?:分|:|：)?(?:\\s*)(\\d+|[" + Object.keys(NUMBER).join("") + "]+)?(?:\\s*)(?:秒)?)(?:\\s*(A.M.|P.M.|AM?|PM?))?", "i");
var DAY_GROUP_1 = 1;
...
var ZHHansTimeExpressionParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    const refMoment = (0, import_dayjs30.default)(context.refDate);
    const result = context.createParsingResult(match.index, match[0]);
    ...
    if (match[SECOND_GROUP4]) {
      let second = parseInt(match[SECOND_GROUP4]);
      if (isNaN(second)) {
        second = zhStringToNumber(match[SECOND_GROUP4]);
      }
      if (second >= 60)
        return null;
      result.start.assign("second", second);
    }
    hour = parseInt(match[HOUR_GROUP4]);
    if (isNaN(hour)) {
      hour = zhStringToNumber(match[HOUR_GROUP4]);
    }
    if (match[MINUTE_GROUP4]) {
      if (match[MINUTE_GROUP4] == "半") {
        minute = 30;
      } else if (match[MINUTE_GROUP4] == "正" || match[MINUTE_GROUP4] == "整") {
        minute = 0;
      } else {
        minute = parseInt(match[MINUTE_GROUP4]);
        if (isNaN(minute)) {
          minute = zhStringToNumber(match[MINUTE_GROUP4]);
        }
      }
    } else if (hour > 100) {
      minute = hour % 100;
...
var PATTERN35 = new RegExp("(?:星期|礼拜|周)(?<weekday>" + Object.keys(WEEKDAY_OFFSET).join("|") + ")");
...
var ZHHantDateParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    const startMoment = (0, import_dayjs33.default)(context.refDate);
    const result = context.createParsingResult(match.index, match[0]);
    let month = parseInt(match[MONTH_GROUP5]);
    if (isNaN(month))
      month = zhStringToNumber2(match[MONTH_GROUP5]);
    result.start.assign("month", month);
    if (match[DAY_GROUP4]) {
      let day = parseInt(match[DAY_GROUP4]);
      if (isNaN(day))
        day = zhStringToNumber2(match[DAY_GROUP4]);
      result.start.assign("day", day);
    } else {
      result.start.imply("day", startMoment.date());
    }
    if (match[YEAR_GROUP14]) {
      ...
      if (isNaN(year3))
        year3 = zhStringToYear2(match[YEAR_GROUP14]);
      result.start.assign("year", year3);
    } else {
      result.start.imply("year", startMoment.year());
    }
...
var PATTERN36 = new RegExp("(\\d+|[" + Object.keys(NUMBER2).join("") + "]+|半|幾)(?:\\s*)(?:個)?(秒(?:鐘)?|分鐘|小時|鐘|日|天|星期|禮拜|月|年)(?:(?:之|過)?後|(?:之)?內)", "i");
var NUMBER_GROUP2 = 1;
var UNIT_GROUP2 = 2;
var ZHHantDeadlineFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    let date = (0, import_dayjs34.default)(context.refDate);
...
var PATTERN37 = new RegExp("(?<prefix>上|今|下|這|呢)(?:個)?(?:星期|禮拜|週)(?<weekday>" + Object.keys(WEEKDAY_OFFSET2).join("|") + ")");
var ZHHantRelationWeekdayParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    let startMoment = (0, import_dayjs35.default)(context.refDate);
    let startMomentFixed = false;
    const refOffset = startMoment.day();
    if (modifier == "last" || modifier == "past") {
      startMoment = startMoment.day(offset - 7);
...
var FIRST_REG_PATTERN4 = new RegExp("(?:由|從|自)?(?:(今|明|前|大前|後|大後|聽|昨|尋|琴)(早|朝|晚)|(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|(今|明|前|大前|後|大後|聽|昨|尋|琴)(?:日|天)(?:[\\s,，]*)(?:(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?)?(?:[\\s,，]*)(?:(\\d+|[" + Object.keys(NUMBER2).join("") + "]+)(?:\\s*)(?:點|時|:|：)(?:\\s*)(\\d+|半|正|整|[" + Object.keys(NUMBER2).join("") + "]+)?(?:\\s*)(?:分|:|：)?(?:\\s*)(\\d+|[" + Object.keys(NUMBER2).join("") + "]+)?(?:\\s*)(?:秒)?)(?:\\s*(A.M.|P.M.|AM?|PM?))?", "i");
var SECOND_REG_PATTERN4 = new RegExp("(?:^\\s*(?:到|至|\\-|\\–|\\~|\\〜)\\s*)(?:(今|明|前|大前|後|大後|聽|昨|尋|琴)(早|朝|晚)|(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨))|(今|明|前|大前|後|大後|聽|昨|尋|琴)(?:日|天)(?:[\\s,，]*)(?:(上(?:午|晝)|朝(?:早)|早(?:上)|下(?:午|晝)|晏(?:晝)|晚(?:上)|夜(?:晚)?|中(?:午)|凌(?:晨)))?)?(?:[\\s,，]*)(?:(\\d+|[" + Object.keys(NUMBER2).join("") + "]+)(?:\\s*)(?:點|時|:|：)(?:\\s*)(\\d+|半|正|整|[" + Object.keys(NUMBER2).join("") + "]+)?(?:\\s*)(?:分|:|：)?(?:\\s*)(\\d+|[" + Object.keys(NUMBER2).join("") + "]+)?(?:\\s*)(?:秒)?)(?:\\s*(A.M.|P.M.|AM?|PM?))?", "i");
var DAY_GROUP_13 = 1;
...
var ZHHantTimeExpressionParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    const refMoment = (0, import_dayjs36.default)(context.refDate);
    const result = context.createParsingResult(match.index, match[0]);
    let startMoment = refMoment.clone();
    ...
    if (match[SECOND_GROUP5]) {
      var second = parseInt(match[SECOND_GROUP5]);
      if (isNaN(second)) {
        second = zhStringToNumber2(match[SECOND_GROUP5]);
      }
      if (second >= 60)
        return null;
      result.start.assign("second", second);
    ...
    if (isNaN(hour)) {
      hour = zhStringToNumber2(match[HOUR_GROUP5]);
    }
    if (match[MINUTE_GROUP5]) {
      if (match[MINUTE_GROUP5] == "半") {
        ...
        if (isNaN(minute)) {
          minute = zhStringToNumber2(match[MINUTE_GROUP5]);
        }
      }
    } else if (hour > 100) {
      minute = hour % 100;
...
var PATTERN38 = new RegExp("(?:星期|禮拜|週)(?<weekday>" + Object.keys(WEEKDAY_OFFSET2).join("|") + ")");
...
var RUMonthNameLittleEndianParser = class extends AbstractParserWithLeftRightBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP15]) {
      const yearNumber = parseYear6(match[YEAR_GROUP15]);
      result.start.assign("year", yearNumber);
    } else {
      const year3 = findYearClosestToRef(context.refDate, day, month);
      result.start.imply("year", year3);
...
var RUMonthNameParser = class extends AbstractParserWithLeftBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP16]) {
      const year3 = parseYear6(match[YEAR_GROUP16]);
      result.start.assign("year", year3);
    } else {
      const year3 = findYearClosestToRef(context.refDate, 1, month);
      result.start.imply("year", year3);
...
var RUCasualTimeParser = class extends AbstractParserWithLeftRightBoundaryChecking {
  ...
  innerExtract(context, match) {
    let targetDate = (0, import_dayjs40.default)(context.refDate);
    const lowerText = match[0].toLowerCase();
    const component = context.createParsingComponents();
    if (lowerText === "сейчас") {
      return now(context.reference);
...
var RURelativeDateFormatParser = class extends AbstractParserWithLeftRightBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    const components = context.createParsingComponents();
    let date = (0, import_dayjs41.default)(context.reference.instant);
...
var PATTERN40 = new RegExp(`(?:(?:\\,|\\(|\\（)\\s*)?(?:(este|esta|pasado|pr[oó]ximo)\\s*)?(${matchAnyPattern(WEEKDAY_DICTIONARY7)})(?:\\s*(?:\\,|\\)|\\）))?(?:\\s*(este|esta|pasado|pr[óo]ximo)\\s*semana)?(?=\\W|\\d|$)`, "i");
var PREFIX_GROUP7 = 1;
...
var PATTERN41 = new RegExp(`([0-9]{1,2})(?:º|ª|°)?(?:\\s*(?:desde|de|\\-|\\–|ao?|\\s)\\s*([0-9]{1,2})(?:º|ª|°)?)?\\s*(?:de)?\\s*(?:-|/|\\s*(?:de|,)?\\s*)(${matchAnyPattern(MONTH_DICTIONARY7)})(?:\\s*(?:de|,)?\\s*(${YEAR_PATTERN7}))?(?=\\W|$)`, "i");
var DATE_GROUP10 = 1;
...
var ESMonthNameLittleEndianParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP17]) {
      const yearNumber = parseYear7(match[YEAR_GROUP17]);
      result.start.assign("year", yearNumber);
    } else {
      const year3 = findYearClosestToRef(context.refDate, day, month);
      result.start.imply("year", year3);
...
var ESCasualTimeParser = class extends AbstractParserWithWordBoundaryChecking {
  ...
  innerExtract(context, match) {
    const targetDate = (0, import_dayjs43.default)(context.refDate);
    const component = context.createParsingComponents();
    switch (match[1].toLowerCase()) {
      case "tarde":
        component.imply("meridiem", Meridiem.PM);
...
var PATTERN42 = `(?:(?:приблизно|орієнтовно)\\s*(?:~\\s*)?)?(${TIME_UNITS_PATTERN7})${REGEX_PARTS2.rightBoundary}`;
var UKTimeUnitWithinFormatParser = class extends AbstractParserWithWordBoundaryChecking {
  patternLeftBoundary() {
    return REGEX_PARTS2.leftBoundary;
  }
  innerPattern(context) {
    return context.option.forwardDate ? new RegExp(PATTERN42, "i") : new RegExp(`(?:протягом|на протязі|протягом|упродовж|впродовж)\\s*${PATTERN42}`, REGEX_PARTS2.flags);
  }
  innerExtract(context, match) {
...
var UKMonthNameLittleEndianParser = class extends AbstractParserWithLeftRightBoundaryChecking2 {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP18]) {
      const yearNumber = parseYearPattern(match[YEAR_GROUP18]);
      result.start.assign("year", yearNumber);
    } else {
...
var UkMonthNameParser = class extends AbstractParserWithLeftBoundaryChecking2 {
  ...
  innerExtract(context, match) {
    ...
    if (match[YEAR_GROUP19]) {
      const year3 = parseYearPattern(match[YEAR_GROUP19]);
      result.start.assign("year", year3);
    } else {
...
var UKCasualTimeParser = class extends AbstractParserWithLeftRightBoundaryChecking2 {
  ...
  innerExtract(context, match) {
    let targetDate = (0, import_dayjs45.default)(context.reference.instant);
    const lowerText = match[0].toLowerCase();
    const component = context.createParsingComponents();
    if (lowerText === "зараз") {
      return now(context.reference);
...
var UKRelativeDateFormatParser = class extends AbstractParserWithLeftRightBoundaryChecking2 {
  ...
  innerExtract(context, match) {
    ...
    const components = context.createParsingComponents();
    let date = (0, import_dayjs46.default)(context.reference.instant);
    if (timeunit.match(/week/i)) {
      date = date.add(-date.get("d"), "d");
      components.imply("day", date.date());
      components.imply("month", date.month() + 1);
